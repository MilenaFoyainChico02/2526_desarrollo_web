...
def obtener_usuario_por_email(email):
    ...
    cursor.execute(f'SELECT id_usuario, nombre, {column}, password FROM usuario WHERE {column} = %s', (email,))
    row = cursor.fetchone()
    cursor.close()
    conn.close()
...
def obtener_usuario_por_id(user_id):
    ...
    cursor.execute('USE cimazon')
    column = get_usuario_email_column(cursor)
    cursor.execute(f'SELECT id_usuario, nombre, {column}, password FROM usuario WHERE id_usuario = %s', (user_id,))
    row = cursor.fetchone()
    cursor.close()
    conn.close()
